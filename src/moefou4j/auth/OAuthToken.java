...
abstract class OAuthToken {
	...
	OAuthToken(final HttpResponse response) throws MoefouException {
		this(response.asString());
	}

	OAuthToken(final String string) {
		responseStr = Moefou4JInternalStringUtil.split(string, "&");
	...
	public String getParameter(final String parameter) {
		...
		for (final String str : responseStr) {
			if (str.startsWith(parameter + '=')) {
				value = Moefou4JInternalStringUtil.split(str, "=")[1].trim();
				break;
			}
		}
		return value;
