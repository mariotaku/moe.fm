...
public class OAuthAuthorization implements Authorization, OAuthSupport {
	 ...
	 */
	@Override
	public AccessToken getOAuthAccessToken(final RequestToken requestToken) throws MoefouException {
	...
	public RequestToken getOAuthRequestToken(final String callbackURL) throws MoefouException {
		...
		oauthToken = new RequestToken(conf, http.post(url, params.toArray(new HttpParameter[params.size()]), this),
				this);
	...
	public String toString() {
		return "OAuthAuthorization{" + "consumerKey='" + consumerKey + '\'' + ", consumerSecret='"
				+ Moefou4JInternalStringUtil.maskString(consumerSecret) + "'" + ", oauthToken=" + oauthToken + '}';
	}
	...
	private void parseGetParameters(final String url, final List<HttpParameter> signatureBaseParams) {
		...
		if (-1 != queryStart) {
			final String[] queryStrs = Moefou4JInternalStringUtil.split(url.substring(queryStart + 1), "&");
			try {
				for (final String query : queryStrs) {
					final String[] split = Moefou4JInternalStringUtil.split(query, "=");
					if (split.length == 2) {
	 ...
	 */
	/* package */String generateAuthorizationHeader(final String method, final String url,
	...
	/* package */String generateAuthorizationHeader(final String method, final String url, HttpParameter[] params,
